      ...
      child: Row(
        children: [
          ...
          CachedNetworkImage(
            ...
            imageBuilder: (context, image) => CircleAvatar(radius: Get.size.width * 0.21, backgroundImage: image),
          ),
          SizedBox(width: Get.size.width * 0.05),
          Column(
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    followingController.followUser(followingUser: widget.user, currentUser: authController.user);
                  },
                  child: GetBuilder<AuthController>(
                      builder: (con) => Text(con.user.followings.contains(widget.user.uid) ? 'Following' : 'Follow')),
              ...
              SizedBox(
                ...
                child: GetBuilder<AuthController>(
                  builder: (_) => ElevatedButton(
                    onPressed: !followingController.checkIfFriend(widget.user.uid)
                        ? () {
                            if (!followingController.checkIfFriend(widget.user.uid)) {
                              followingController.addUserAsFriend(
                                  followingUser: widget.user, currentUser: authController.user);
                            }
                          }
                        : null,

                    child: followingController.checkIfFriend(widget.user.uid)
                        ? Row(
              ...
              //   child: ElevatedButton(
              //     onPressed: () {
              //       followingController.addUserAsFriend(followingUser: widget.user, currentUser: authController.user);
              //     },
